...
export default function FacialManagementScreen() {
  ...
  const fichaOptions = allFichas.map((f) => ({
    value: f.id,
    ...
    <View style={[fs.safe, { backgroundColor: bg }]}>
      ...
      <ScrollView contentContainerStyle={fs.scroll}>
        <View style={fs.centerWrap}>
          <View
            style={[fs.card, { backgroundColor: cardBg, borderColor: border }]}
          >
            <View
              style={[fs.iconWrap, { backgroundColor: theme.primary + "18" }]}
            >
              <Ionicons name="scan-outline" size={28} color={theme.primary} />
            </View>
            <Text style={[fs.cardTitle, { color: text }]}>
              {t("facial.setup.title")}
            </Text>
            <Text style={[fs.cardSubtitle, { color: muted }]}>
              {t("facial.setup.helper")}
            </Text>
