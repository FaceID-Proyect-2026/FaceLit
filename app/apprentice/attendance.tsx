...
export default function ApprenticeAttendanceScreen() {
  ...
  function statusLabel(status: AttendanceStatus, delay: number): string {
    if (status === 'late')       return `${t('apprentice.attendance.statuses.late')} · ${delay} min`;
    if (status === 'absent')     return t('apprentice.attendance.statuses.absent');
    if (status === 'invalidEnv') return t('apprentice.attendance.statuses.invalidEnv');
    return t('apprentice.attendance.statuses.punctual');
  }
          ...
          {stats.totalDelayMin > 0 && (
            <View style={[s.delayRow, { borderTopColor: rowDiv }]}>
              ...
              <Text style={[s.delayText, { color: muted }]}>
                {t('apprentice.attendance.totalDelay')}: {formatDuration(stats.totalDelayMin)}
              </Text>
            </View>
          )}
        </View>
          ...
          {(['all', 'lastMonth', 'lastQuarter', 'lastYear'] as FilterPeriod[]).map(p => (
            ...
            >
              <Text style={{
                ...
                {p === 'all' ? t('apprentice.attendance.allPeriods') : t(`apprentice.attendance.${p}`)}
              </Text>
            </TouchableOpacity>
          ))}
        </ScrollView>
        ...
        {myEvents.length === 0 && (
          <View style={s.empty}>
            ...
            <Text style={[s.emptyText, { color: muted }]}>
              {t('apprentice.attendance.noRecordsPeriod')}
            </Text>
          </View>
        )}
              ...
              >
                <View style={s.monthHeaderLeft}>
                  ...
                  <Text style={[s.monthCount, { color: muted }]}>
                    {group.events.length} {t('apprentice.attendance.classes')}
                  </Text>
                </View>
                  ...
                  <View key={evt.id}>
                      ...
                      <View style={s.rowContent}>
                        <View style={s.rowTop}>
                          <Text style={[s.rowDate, { color: text }]}>
                            {formatDateShort(evt.date, i18n.language)}
                          </Text>
                    ...
                    {isExpanded && (
                      <View style={[s.detailPanel, { backgroundColor: theme.primary + '08', borderColor: border }]}>
                        <DetailRow icon="calendar-outline"  label={t('apprentice.attendance.dateLabel')}        value={formatDateLong(evt.date, i18n.language)}                  textColor={text}          mutedColor={muted} />
                        {evt.entryTime   ? <DetailRow icon="log-in-outline"   label={t('apprentice.attendance.entryTime')}     value={evt.entryTime}                             textColor={text}          mutedColor={muted} /> : null}
                        {evt.exitTime    ? <DetailRow icon="log-out-outline"  label={t('apprentice.attendance.exitTime')}      value={evt.exitTime}                              textColor={text}          mutedColor={muted} /> : null}
                        {evt.durationMin !== null
                          ? <DetailRow icon="hourglass-outline" label={t('apprentice.attendance.timeInClass')} value={formatDuration(evt.durationMin)}             textColor={text}          mutedColor={muted} />
                          : null}
                        {evt.scheduleStart !== '—'
                          ? <DetailRow icon="time-outline"      label={t('apprentice.attendance.scheduledHours')} value={`${evt.scheduleStart} – ${evt.scheduleEnd}`} textColor={text}      mutedColor={muted} />
                          : null}
                        <DetailRow icon="business-outline"  label={t('apprentice.attendance.environmentLabel')} value={evt.environmentCode}                        textColor={text}          mutedColor={muted} />
                        <DetailRow icon="person-outline"    label={t('apprentice.attendance.instructorLabel')}  value={evt.instructorName}                         textColor={text}          mutedColor={muted} />
                        {evt.status === 'late' && evt.delayMinutes > 0
                          ? <DetailRow icon="alert-circle-outline" label={t('apprentice.attendance.delay')} value={`${evt.delayMinutes} min`}               textColor={Colors.warning} mutedColor={muted} />
                          : null}
                      </View>
                    )}
                  </View>
