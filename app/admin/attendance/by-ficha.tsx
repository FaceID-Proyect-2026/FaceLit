...
export default function AttendanceByFichaScreen({
  ...
  const handleExport = async (format: "excel" | "csv") => {
    ...
    try {
      ...
      const headers = [
        t("attendance.rf6.learner"),
        t("attendance.rf6.document"),
        ...dates.map(fmtDate),
      ];
      const rows = tableRows.map((row) => [
        ...
        ...row.days.map((cell) => {
          ...
          if (cell.status === "late")
            return `${t("attendance.statuses.late")} (${cell.delayMinutes} min)`;
    ...
    >
      ...
      {!selectedFichaId && !directFichaOnly && (
        <View
          style={[
            s.card,
            { backgroundColor: cardBg, borderColor: border, zIndex: 10 },
          ]}
        >
          <SearchableSelect
      ...
      {!selectedProgramId && !selectedFichaId && !directFichaOnly && (
        <View style={s.prompt}>
          ...
          <Text style={[s.promptText, { color: muted }]}>
            {t("attendance.rf6.selectProgramPrompt")}
          </Text>
        </View>
      )}
      ...
      {selectedProgramId && !selectedFichaId && fichaCards.length === 0 && (
        <View style={s.emptyBox}>
          ...
          <Text style={[s.emptyText, { color: muted }]}>
            {t("attendance.noFichasInProgram")}
          </Text>
        </View>
      )}
              ...
              >
                ...
                >
                  <Ionicons
                    ...
                    color={theme.primary}
                  />
                  <Text style={[s.fichaNumber, { color: theme.primary }]}>
                    {card.fichaNumber}
                  </Text>
                </View>
                <View style={s.fichaMetrics}>
                  <View style={s.fichaMetric}>
                    <View
                      style={[s.metricDot, { backgroundColor: Colors.error }]}
                    />
                    <Text style={[s.metricPct, { color: Colors.error }]}>
                      {card.absentPct}%
                    </Text>
                    <Text style={[s.metricLabel, { color: muted }]}>
                      {t("attendance.rf6.absent")}
                    </Text>
                  </View>
                  <View
                    style={[s.metricDivider, { backgroundColor: border }]}
                  />
                  <View style={s.fichaMetric}>
                    <View
                      style={[s.metricDot, { backgroundColor: Colors.warning }]}
                    />
                    <Text style={[s.metricPct, { color: Colors.warning }]}>
                      {card.latePct}%
                    </Text>
                    <Text style={[s.metricLabel, { color: muted }]}>
                      {t("attendance.rf6.late")}
                    </Text>
                  </View>
                </View>
                <Text style={[s.fichaTotal, { color: muted }]}>
                  {card.absentToday} {t("attendance.rf6.absentCount")} ·{" "}
                  {card.lateToday} {t("attendance.rf6.lateCount")} ·{" "}
                  {card.totalLearners} {t("attendance.rf6.total")}
                </Text>
                <View style={[s.fichaAction, { borderTopColor: border }]}>
                  <Text style={[s.fichaActionText, { color: theme.primary }]}>
                    {t("attendance.rf6.viewDetail")}
                  </Text>
                  <Ionicons
      ...
      {selectedFichaId && (
        <>
          ...
          >
            <Text style={[s.rangeLabel, { color: muted }]}>
              {t("attendance.rf6.selectRange")}
            </Text>
              ...
              >
                <View>
                  <View style={s.tableHeaderRow}>
                    ...
                    >
                      <Text style={[s.thText, { color: theme.primary }]}>
                        {t("attendance.rf6.learner")}
                      </Text>
                    </View>
                    {dates.map((d) => (
                  ...
                  {tableRows.map((row, rIdx) => (
                    <View
                      ...
                      style={[
                        ...
                        {
                          backgroundColor:
                            rIdx % 2 === 0 ? cardBg : theme.primary + "08",
                        },
                      ]}
                    >
                      <View style={[s.cellName, { borderColor: border }]}>
                        <Text
                          style={[s.cellNameText, { color: text }]}
                          numberOfLines={1}
                        >
                          {row.learnerName}
                        </Text>
                        <Text
                          style={[s.cellDocText, { color: muted }]}
                          numberOfLines={1}
                        >
                          {row.learnerDocument}
                        </Text>
                        ...
                        return (
                          <TouchableOpacity
                            ...
                            style={[
                              ...
                              { borderColor: border },
                              isAbsent && {
                                backgroundColor: CELL_ABSENT,
                                borderColor: BORDER_ABSENT,
                              },
                              isLate && {
                                backgroundColor: CELL_LATE,
                                borderColor: BORDER_LATE,
                              },
                            ]}
                          ...
                          >
                            ...
                            {!cell.status && (
                              <Text style={[s.cellEmpty, { color: muted }]}>
                                —
                              </Text>
                            )}
                          </TouchableOpacity>
              ...
              <View style={s.legend}>
                {[
                  ...
                  },
                  {
                    ...
                    label: t("attendance.statuses.punctual"),
                  },
                ].map((item) => (
          ...
          {(!dateFrom || !dateTo) && (
            <View style={s.emptyBox}>
              ...
              <Text style={[s.emptyText, { color: muted }]}>
                {t("attendance.rf6.selectRangePrompt")}
              </Text>
            </View>
          )}
          ...
          {dateFrom &&
            ...
            tableRows.length === 0 && (
              <View style={s.emptyBox}>
                ...
                />
                <Text style={[s.emptyText, { color: muted }]}>
                  {t("attendance.rf6.noRecordsRange")}
                </Text>
              </View>
            )}
        </>
                ...
                {(
                  [
                    [
                      "person-outline",
                      t("attendance.rf6.learner"),
                      cellDetail.learnerName,
                    ],
                    [
                      "calendar-outline",
                      t("attendance.fields.date"),
                      fmtDateLong(cellDetail.cell.date),
                    ],
                    [
                      "log-in-outline",
                      t("attendance.fields.entryTime"),
                      fmtTime(cellDetail.cell.entryTime),
                    ],
                    ...(cellDetail.cell.delayMinutes > 0
                      ? [
                          [
                            "timer-outline",
                            t("attendance.fields.delay"),
                            `${cellDetail.cell.delayMinutes} min`,
                          ],
                        ]
                      : []),
                    [
                      "business-outline",
                      t("attendance.fields.environment"),
                      cellDetail.cell.environmentName || "—",
                    ],
                    [
                      "person-circle-outline",
                      t("attendance.fields.instructor"),
                      cellDetail.cell.instructorName || "—",
                    ],
                  ] as [string, string, string][]
                ).map(([icon, label, value]) => (
                  <View
                    key={label}
                    style={[s.detailRow, { borderBottomColor: border }]}
                  >
                    <View style={s.detailLabel}>
                      <Ionicons name={icon as any} size={15} color={muted} />
                      <Text style={[s.detailLabelText, { color: muted }]}>
                        {label}
                      </Text>
                    </View>
