...
function OnboardingModern({ isDark, theme }: { isDark: boolean; theme: any }) {
  ...
  const steps = [
    ['document-text-outline', t('academic.academicStep1Title'), t('academic.academicStep1Desc')],
    ['create-outline', t('academic.academicStep2Title'), t('academic.academicStep2Desc')],
    ['cloud-upload-outline', t('academic.academicStep3Title'), t('academic.academicStep3Desc')],
  ];
  ...
  return (
    <Animated.View style={[modern.wrap, { opacity: fade, transform: [{ translateY: lift }] }]}>
      <LinearGradient colors={isDark ? ['#10281B', '#08150F', '#050B08'] : ['#EAF8EC', '#F8FCF8', '#FFFFFF']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[modern.panel, { borderColor: border }]}>
        ...
        <View style={[modern.hero, compact && modern.heroCompact]}>
          <View style={modern.heroCopy}>
            <View style={[modern.eyebrow, { backgroundColor: soft, borderColor: border }]}>
              ...
              <Text style={[modern.eyebrowText, { color: theme.primary }]}>{t('academic.academicInitialSetup')}</Text>
            </View>
            <Text style={[modern.title, { color: text }]}>{t('academic.academicOnboardingTitle')}</Text>
            <Text style={[modern.subtitle, { color: muted }]}>{t('academic.academicOnboardingSubtitle')}</Text>
            <View style={modern.benefits}>
              {[t('academic.academicBenefit1'), t('academic.academicBenefit2'), t('academic.academicBenefit3')].map(item => (
        ...
        <Text style={[modern.kicker, { color: theme.primary }]}>{t('academic.academicHowItWorks')}</Text>
        <Text style={[modern.sectionTitle, { color: text }]}>{t('academic.academicThreeSteps')}</Text>
        ...
        <View style={[modern.action, { backgroundColor: isDark ? '#0A1A11' : '#F1FAF2', borderColor: border }]}>
          <View style={modern.actionCopy}><Text style={[modern.actionTitle, { color: text }]}>{t('academic.academicReadyToStart')}</Text><Text style={[modern.actionDesc, { color: muted }]}>{t('academic.academicReadyToStartDesc')}</Text></View>
          <TouchableOpacity onPress={() => router.push('/admin/academic/csv-upload' as any)} activeOpacity={0.85} style={modern.buttonWrap}>
            <LinearGradient colors={['#72C96D', '#55AD52', '#3D8F43']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={modern.button}>
              <Ionicons name={'cloud-upload-outline'} size={21} color={Colors.white} /><Text style={modern.buttonText}>{t('academic.academicLoadCsvBtn')}</Text><Ionicons name={'arrow-forward'} size={19} color={Colors.white} />
            </LinearGradient>
          </TouchableOpacity>
        </View>
        <View style={modern.hint}><Ionicons name={'information-circle-outline'} size={17} color={muted} /><Text style={{ color: muted, fontSize: FontSize.xs }}>{t('academic.academicManualRegisterHint')}</Text></View>
      </LinearGradient>
    </Animated.View>
  );
}
...
export default function AcademicProgramsScreen() {
  ...
  const handleDeactivate = (id: string, name: string) => {
    alert(t('academic.programDelete'), `${name}\n\n${t('academic.programDeactivateConfirm')}`, [
      ...
      { text: t('academic.programDelete'), style: 'destructive', onPress: async () => {
        try { await deactivateProgram(id); } catch (error: any) { alert(t('common.error'), error?.response?.data?.message ?? t('academic.programUpdateError')); }
      }},
    ]);
  };
  ...
  const handleDeleteCompletely = (id: string, name: string) => {
    alert(t('academic.programDeleteCompletely'), `${name}\n\n${t('academic.programDeleteCompletelyConfirm')}`, [
      ...
      { text: t('common.yes'), style: 'destructive', onPress: async () => {
        ...
        catch (error: any) { alert(t('common.error'), error?.response?.data?.message ?? t('academic.programDeleteError')); }
      }},
    ]);
  };
  ...
  const handleReactivate = (id: string, name: string) => {
    alert(t('academic.alreadyActive'), `${name}\n\n${t('environments.reactivateConfirm')}`, [
      ...
      { text: t('environments.reactivate'), onPress: async () => {
        try { await reactivateProgram(id); } catch (error: any) { alert(t('common.error'), error?.response?.data?.message ?? t('academic.programReactivateError')); }
      } },
    ]);
  };
  ...
  const handleReactivateFicha = (id: string, number: string) => {
    alert(t('academic.alreadyActive'), `${number}\n\n${t('environments.reactivateConfirm')}`, [
      { text: t('common.cancel'), style: 'cancel' },
      { text: t('environments.reactivate'), onPress: async () => {
        try { await reactivateFicha(id); } catch (error: any) { alert(t('common.error'), error?.response?.data?.message ?? t('academic.fichaReactivateError')); }
      } },
    ]);
  };
  ...
  const handleDeactivateFicha = (id: string, number: string) => {
    alert(t('academic.deactivateFicha'), `${number}\n\n${t('academic.confirmDeactivateFicha')}`, [
      ...
      { text: t('academic.deactivateFicha'), style: 'destructive', onPress: async () => {
        try { await deactivateFicha(id); } catch (error: any) { alert(t('common.error'), error?.response?.data?.message ?? t('academic.fichaUpdateError')); }
      } },
    ]);
  };
